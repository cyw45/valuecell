...
class SpotRsiLadderCandleFeatureComputer(CandleBasedFeatureComputer):
    ...
    def compute_features(
        ...
        for symbol, series in grouped.items():
            ...
            prev2_bb_middle = _to_float(prev2.get("bb_middle"))
            ...
            values = {
                ...
                "rsi_uptrend_3bar": bool(
                    last_rsi is not None
                    and prev_rsi is not None
                    and prev2_rsi is not None
                    and last_rsi > prev_rsi > prev2_rsi
                ),
                "mtm14": last_mtm,
                "prev_mtm14": prev_mtm,
                "mtm_turn_up": bool(
                ...
                "low_uptrend_3bar": bool(
                    float(last.low) > float(prev.low) > float(prev2.low)
                ),
                ...
                "bb_mid_cross_up_confirmed": bool(
                    prev2_bb_middle is not None
                    and prev_bb_middle is not None
                    and last_bb_middle is not None
                    and float(prev2.close) <= prev2_bb_middle
                    and float(prev.close) > prev_bb_middle
                    and float(last.close) > last_bb_middle
                ),
                "price_above_bb_middle": bool(
                    last_bb_middle is not None and float(last.close) > last_bb_middle
                ),
