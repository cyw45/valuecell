...
import valuecell.server.api.routers.rule_strategy as rule_strategy_router
...
def _pnl_curve_client(monkeypatch) -> tuple[TestClient, InMemoryRuleStrategyRepository]:
    repository = InMemoryRuleStrategyRepository()
    monkeypatch.setattr(
        rule_strategy_router, "RuleStrategyRepository", lambda: repository
    )
    return _client(repository), repository


def test_pnl_curve_returns_initial_capital_baseline_before_any_evaluation(monkeypatch):
    client, _ = _pnl_curve_client(monkeypatch)
    assert (
        client.post(
            "/rule-strategies",
            json={
...
def test_pnl_curve_places_one_baseline_before_chronological_journals(monkeypatch):
    client, repository = _pnl_curve_client(monkeypatch)
    assert (
        client.post(
            "/rule-strategies",
            json={
