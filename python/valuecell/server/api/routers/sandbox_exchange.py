...
def create_sandbox_exchange_router() -> APIRouter:
    ...
    def require_sandbox_read(principal: CurrentPrincipal) -> None:
        require_active_tenant(principal)
        require_tenant_permission(principal, "tenant.read")

    def require_sandbox_connection_manage(principal: CurrentPrincipal) -> None:
        require_active_tenant(principal)
        require_tenant_permission(principal, "connection.manage")

    def require_sandbox_trade(principal: CurrentPrincipal) -> None:
        require_active_tenant(principal)
    ...
    async def create_connection(
        ...
        require_sandbox_connection_manage(principal)
        validation = await SandboxExchangeService().validate(
        ...
        try:
            ...
            raise HTTPException(
                status_code=422,
                detail="Sandbox connection label already exists or is invalid",
            ) from exc
    ...
    async def list_connections(
        ...
        return SuccessResponse.create(
            data=SandboxExchangeTradingService(db).connection_metadata(
                principal.tenant_id
            ),
            msg="Sandbox connection metadata retrieved",
        )

    @router.get(
        "/connections/{credential_id}/balance", response_model=SuccessResponse[dict]
    )
    async def connection_balance(
        ...
        require_sandbox_read(principal)
        try:
            data = await SandboxExchangeTradingService(db).balance(
    ...
    async def create_order(
        ...
        require_sandbox_trade(principal)
        if (
            request.idempotency_key
            and idempotency_header
            and request.idempotency_key != idempotency_header
        ):
            raise HTTPException(
                status_code=422, detail="Idempotency key header and body must match"
            )
        client_order_id = idempotency_header or request.idempotency_key
        if client_order_id is None:
            raise HTTPException(
                status_code=422, detail="An idempotency key is required"
            )
        try:
            data = await SandboxExchangeTradingService(db).submit_order(
                principal.tenant_id,
    ...
    async def order_status(
        ...
        require_sandbox_read(principal)
        try:
            data = await SandboxExchangeTradingService(db).fetch_order_status(
