...
def _ensure_system_env_and_load() -> None:
    """Load environment variables from repo-local runtime env and system env.

    Behavior:
    - Prefer repo-local `docker/runtime/.env` during development and deployment.
    - Fall back to system `.env` under the ValueCell config directory.
    - If neither exists, bootstrap the system `.env` from repo `.env.example`.
    - Later files override earlier ones so an explicit runtime env wins.
    """
    try:
        repo_root = Path(__file__).resolve().parents[4]
        sys_env = get_system_env_path()
        runtime_env = repo_root / "docker" / "runtime" / ".env"
        example_file = repo_root / ".env.example"

        try:
            import shutil

            if not runtime_env.exists() and not sys_env.exists() and example_file.exists():
