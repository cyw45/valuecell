...
class AgentOrchestrator:
    ...
    async def _handle_new_request(
        ...
        if user_input.target_agent_name == self.super_agent_service.name:
            ...
            elif super_outcome.reason:
                reason_message = self.event_service.factory.message_response_general(
                    StreamResponseEvent.MESSAGE_CHUNK,
                    conversation_id,
                    thread_id,
                    task_id=generate_task_id(),
                    content=super_outcome.reason,
                    agent_name=self.super_agent_service.name,
                )
                yield await self.event_service.emit(reason_message)
