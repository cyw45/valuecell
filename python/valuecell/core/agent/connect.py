...
async def _wait_for_local_agent_ready(
    ctx: AgentContext,
    *,
    timeout_s: float = 30.0,
    probe_interval_s: float = 0.25,
) -> None:
    """Wait until a freshly launched local agent can serve its agent card.

    Local in-process agents are launched via `asyncio.create_task`, so there is
    a small window where the runtime task exists but uvicorn has not yet bound
    its socket. Probing readiness here prevents the orchestrator from racing
    ahead into client initialization and failing on a transient connection
    error.
    """
    ...
    candidate_urls = _build_local_agent_candidate_urls(url)
    ...
    while loop.time() < deadline:
        if ctx.agent_task and ctx.agent_task.done():
            try:
                ...
                raise RuntimeError(
                    f"Agent '{ctx.name}' failed before becoming ready"
                ) from exc

        async with httpx.AsyncClient(timeout=1.0) as client:
            for candidate_url in candidate_urls:
                try:
                    await _probe_local_agent_card(client, candidate_url)
                    ctx.url = candidate_url
                    if ctx.local_agent_card is not None:
                        ctx.local_agent_card.url = candidate_url
                    logger.info(
                        "Local agent '{}' is ready at {}", ctx.name, candidate_url
                    )
                    return
                except Exception as exc:
                    last_error = exc
    ...
    raise RuntimeError(
        f"Timed out waiting for local agent '{ctx.name}' to become ready at {url}"
    ) from last_error


def _build_local_agent_candidate_urls(url: str) -> list[str]:
    """Build candidate base URLs for local agent probing.

    On Windows, `localhost` can introduce transient resolution or binding
    mismatches during cold start. For local agents we proactively retry the
    loopback IPv4 address as a fallback.
    """
...
def _canonicalize_local_agent_url(url: str | None) -> str | None:
    """Normalize local loopback URLs to IPv4 loopback for Windows stability."""
...
async def _probe_local_agent_card(client: httpx.AsyncClient, base_url: str) -> None:
    """Probe a local agent until either resolver or direct card fetch succeeds."""
    ...
    try:
        ...
        direct_probe_error: Exception | None = None
        for path in ("/.well-known/agent-card.json", "/.well-known/agent.json"):
            try:
                ...
                direct_probe_error = exc

        raise RuntimeError(
            f"Failed to probe local agent card at {base_url}"
        ) from direct_probe_error or resolver_exc
...
class RemoteConnections:
        ...
        for json_file in agent_card_dir.glob("*.json"):
            try:
                ...
                if not local_agent_card:
                    continue
                canonical_url = _canonicalize_local_agent_url(local_agent_card.url)
                if canonical_url:
                    local_agent_card.url = canonical_url
    ...
    async def _ensure_agent_runtime(self, ctx: AgentContext) -> None:
        ...
        if ctx.agent_task is None:
            ...
            # Give the event loop a chance to schedule startup work, then
            # actively wait for the embedded HTTP server to publish its card.
            ...
            if ctx.agent_task.done():
                try:
                    ...
                    ctx.agent_task = None
                    ctx.agent_instance = None
            await _wait_for_local_agent_ready(ctx)
