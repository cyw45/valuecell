...
def test_build_local_agent_candidate_urls_rewrites_localhost():
    candidates = connect_mod._build_local_agent_candidate_urls(
...
def test_load_from_dir_canonicalizes_localhost_url(tmp_path: Path):
    dir_path = tmp_path / "agent_cards"
    dir_path.mkdir(parents=True)

    card = make_card_dict(
        "CanonicalAgent", "http://localhost:9010/", push_notifications=False
    )
    with open(dir_path / "CanonicalAgent.json", "w", encoding="utf-8") as f:
        json.dump(card, f)

    rc = RemoteConnections()
    rc.load_from_dir(str(dir_path))

    ctx = rc._contexts["CanonicalAgent"]
    assert ctx.url == "http://127.0.0.1:9010/"
    assert ctx.local_agent_card is not None
    assert ctx.local_agent_card.url == "http://127.0.0.1:9010/"


@pytest.mark.asyncio
async def test_wait_for_local_agent_ready_falls_back_to_loopback(
    monkeypatch: pytest.MonkeyPatch,
):
    class FakeResolver:
        def __init__(self, _client, url: str):
            self.url = url

        async def get_agent_card(self):
            if self.url == "http://localhost:10008/":
                raise RuntimeError("resolver cold start")
            return {"name": "LocalAgent"}
    ...
    class FakeAsyncClient:
        ...
        async def get(self, _url: str):
            raise AssertionError("direct probe should not be needed in this test")

    monkeypatch.setattr(connect_mod, "A2ACardResolver", FakeResolver)
    monkeypatch.setattr(connect_mod.httpx, "AsyncClient", FakeAsyncClient)
    ...
    await connect_mod._wait_for_local_agent_ready(
        ctx, timeout_s=0.5, probe_interval_s=0.01
    )

    assert ctx.url == "http://127.0.0.1:10008/"


@pytest.mark.asyncio
async def test_probe_local_agent_card_accepts_direct_well_known_response(
    monkeypatch: pytest.MonkeyPatch,
):
    class FakeResolver:
        def __init__(self, _client, _url: str):
            pass
        ...
        def raise_for_status(self):
            if self.status_code >= 400:
                raise httpx.HTTPStatusError(
                    "bad status",
                    request=httpx.Request("GET", "http://127.0.0.1:10008"),
    ...
    class FakeAsyncClient:
        async def get(self, url: str):
            if url.endswith("/.well-known/agent-card.json"):
                return FakeResponse(200, {"name": "LocalAgent"})
            return FakeResponse(404, {})

    monkeypatch.setattr(connect_mod, "A2ACardResolver", FakeResolver)

    await connect_mod._probe_local_agent_card(
        FakeAsyncClient(), "http://127.0.0.1:10008/"
    )
