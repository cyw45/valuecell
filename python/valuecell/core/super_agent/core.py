...
class SuperAgent:
    ...
    _STREAM_TIMEOUT_SECONDS: float = 20.0
    ...
    async def run(
        ...
        try:
            ...
            async for item in self._consume_agent_stream(
                response_stream, user_input.query, model_description
            ):
                yield item
        except Exception as e:
            yield SuperAgentOutcome(
                ...
                reason="SuperAgent execution failed",
            )
    ...
    async def _consume_agent_stream(
        ...
        """Consume the underlying agent stream with a timeout-based fallback.

        Some OpenAI-compatible gateways return HTTP 200 for chat completions but
        can still stall before agno produces the final structured object. When
        that happens we should not leave the frontend in an infinite thinking
        state. Instead, we fall back to planner handoff after a short timeout.
        """
        while True:
            try:
                ...
                yield SuperAgentOutcome(
                    ...
                    reason=(
                        "SuperAgent timed out while waiting for a structured result; "
                        "routing to Planner."
                    ),
                )
                return
            ...
            if not isinstance(final_outcome, SuperAgentOutcome):
                ...
                yield SuperAgentOutcome(
                    ...
                    reason="SuperAgent returned malformed structured output",
                )
                return
        ...
        yield SuperAgentOutcome(
            decision=SuperAgentDecision.HANDOFF_TO_PLANNER,
            enriched_query=original_query,
            reason="SuperAgent returned no final outcome; routing to Planner.",
        )
