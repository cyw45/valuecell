...
import { api } from "../api";
import { BottomSheetSelector, EquityCurveChart, ScreenHeader, StatePanel } from "../components";
...
export default function FundingPnlScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl onRefresh={() => void Promise.all([pnl.refetch(), funding.refetch()])} refreshing={pnl.isRefetching || funding.isRefetching} tintColor={palette.primary} />} style={styles.page}>
    <ScreenHeader actionLabel="切换策略" onAction={() => setPickerVisible(true)} subtitle="服务端按 UTC 日结聚合的账户权益与每日收益" title="资金费与 PnL" />
    ...
    <View style={styles.chartCard}><Text style={styles.cardTitle}>每日收益曲线</Text>{pnl.data?.length ? <EquityCurveChart formatQuote={formatQuote} formatTimestamp={formatTimestamp} height={180} points={pnl.data} /> : <Text style={styles.empty}>服务端尚无日结账户事实；完成首次账户快照后显示曲线。</Text>}</View>
