...
const RULE_BASED_STRATEGY_TYPES = [
  "LongTermSpotRsiStrategy",
  "ShortTermSpotRsiStrategy",
  "ColdAltSpotStrategy",
] as const;

type RuleBasedStrategyType = (typeof RULE_BASED_STRATEGY_TYPES)[number];

const isRuleBasedStrategyType = (
  value: Strategy["strategy_type"],
): value is RuleBasedStrategyType =>
  RULE_BASED_STRATEGY_TYPES.includes(value as RuleBasedStrategyType);
        ...
        <form.AppField
          listeners={{
            onChange: ({ value }: { value: Strategy["strategy_type"] }) => {
              ...
              if (isRuleBasedStrategyType(value)) {
                form.setFieldValue(
                  "symbols",
                  value === "ColdAltSpotStrategy" ? [] : TRADING_SYMBOLS,
                );
                form.setFieldValue("max_leverage", 1);
                form.setFieldValue("template_id", "");
                form.setFieldValue(
                  "decide_interval",
                  value === "LongTermSpotRsiStrategy"
                    ? 300
                    : value === "ColdAltSpotStrategy"
                      ? 3600
                      : 60,
                );
                return;
        ...
        >
          {(field) => (
            <field.SelectField label={t("strategy.form.strategyType.label")}>
              ...
              <SelectItem value="LongTermSpotRsiStrategy">
                {t("strategy.form.strategyType.longTermSpotRsi")}
              </SelectItem>
              <SelectItem value="ShortTermSpotRsiStrategy">
                {t("strategy.form.strategyType.shortTermSpotRsi")}
              </SelectItem>
              <SelectItem value="ColdAltSpotStrategy">
                {t("strategy.form.strategyType.coldAltSpot")}
              </SelectItem>
            </field.SelectField>
          )}
        </form.AppField>
