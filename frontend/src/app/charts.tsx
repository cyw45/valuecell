...
export default function ChartsPage() {
  ...
  const fromTsMs = useMemo(() => {
    ...
    const days =
      HISTORY_RANGES.find((range) => range.value === historyRange)?.days ?? 10;
  ...
  const lookback = useMemo(
    () =>
      Math.min(
        5_000,
        Math.ceil((toTsMs - fromTsMs) / (INTERVAL_SECONDS[interval] * 1000)) +
          2,
      ),
  ...
  const movingAverages = useMemo<CandlestickMovingAverage[]>(() => {
    ...
    return overlays.map((overlay) => ({
      name: overlay.name,
      color: overlay.color,
      values: market.indicators.map(
        (indicator) => indicator.ma[overlay.key] ?? null,
      ),
    }));
  }, [market]);
          ...
          <CardHeader className="gap-4 sm:flex sm:flex-row sm:items-end sm:justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <BarChart3 className="size-5" /> {t("saas.charts.marketChart")}
              </CardTitle>
              <CardDescription className="mt-1">
                {market?.latest_price != null
                  ? t("saas.charts.latestPrice", {
            ...
            <div className="flex flex-wrap gap-1" aria-label="Historical range">
              {HISTORY_RANGES.map((range) => (
                <Button
                  ...
                  size="sm"
                  type="button"
                  variant={historyRange === range.value ? "secondary" : "ghost"}
                >
                  {range.label}
                </Button>
              ))}
                ...
                <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                      Derived from the same exchange OHLCV snapshot as the
                      candlestick chart.
                    </p>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <Tabs
                      onValueChange={(value) =>
                        setIndicatorPanel(value as MarketIndicatorPanel)
                      }
                      value={indicatorPanel}
                    >
                    ...
                    {indicatorPanel === "rsi" ? (
                      <Tabs
                        onValueChange={(value) =>
                          setRsiMode(value as RsiBollingerMode)
                        }
                        value={rsiMode}
                      >
          ...
          <CardContent className="flex flex-col gap-3 px-5 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <a
                ...
                target="_blank"
              >
                {t("saas.charts.openTradingView")} <ExternalLink />
              </a>
            </Button>
          </CardContent>
        </Card>
