...
const ADVANCED_TIMEFRAME_OPTIONS = [
  "1m",
  "3m",
  "5m",
  "15m",
...
function IntervalSelect({
  ...
  return (
    <Select disabled={disabled} value={value} onValueChange={onChange}>
      ...
      <SelectContent>
        {ADVANCED_TIMEFRAME_OPTIONS.map((interval) => (
...
function AdvancedThresholdEditor({
  ...
  onChange: (
    changes: Partial<AdvancedRuleSetConfig["rsi"]> & {
      component?: "ar" | "br";
    },
  ) => void;
  brar?: boolean;
    ...
    <div className="grid gap-3 rounded-md border border-border/70 p-3">
      ...
      <div
        className={cn(
          ...
          brar ? "lg:grid-cols-5" : "lg:grid-cols-4",
        )}
      >
        <div className="grid gap-1.5">
          <Label>周期</Label>
          <IntervalSelect
            ...
            id={`${label}-interval`}
            onChange={(interval) => onChange({ interval: interval as "15m" })}
            value={rule.interval}
          />
        </div>
        <NumericField
        ...
        {brar ? (
          <div className="grid gap-1.5">
            ...
            <Select
              disabled={!rule.enabled}
              value={rule.component ?? "br"}
              onValueChange={(component) =>
                onChange({ component: component as "ar" | "br" })
              }
            >
              <SelectTrigger>
                <SelectValue />
        ...
        <div className="grid gap-1.5">
          ...
          <Select
            disabled={!rule.enabled}
            value={rule.entry_comparator}
            onValueChange={(entry_comparator) =>
              onChange({
                entry_comparator: entry_comparator as "above" | "below",
              })
            }
          >
            <SelectTrigger>
        ...
        <NumericField
          ...
          max={100_000_000}
          min={-100_000_000}
          onChange={(entry_threshold) => onChange({ entry_threshold })}
          step={0.1}
          value={rule.entry_threshold}
        />
      </div>
      <div className="grid gap-3 border-border/60 border-t pt-3 sm:grid-cols-4">
        <div className="flex items-center justify-between gap-3">
          <Label htmlFor={`${label}-exit`}>启用卖出阈值</Label>
          <Switch
            checked={rule.exit_enabled}
            disabled={!rule.enabled}
            id={`${label}-exit`}
            onCheckedChange={(exit_enabled) => onChange({ exit_enabled })}
          />
        </div>
        <div className="grid gap-1.5">
          <Label>卖出比较</Label>
          <Select
            disabled={!rule.enabled || !rule.exit_enabled}
            value={rule.exit_comparator}
            onValueChange={(exit_comparator) =>
              onChange({
                exit_comparator: exit_comparator as "above" | "below",
              })
            }
          >
            <SelectTrigger>
        ...
        <NumericField
          ...
          max={100_000_000}
          min={-100_000_000}
          onChange={(exit_threshold) => onChange({ exit_threshold })}
          step={0.1}
          value={rule.exit_threshold}
        />
      </div>
    </div>
  );
    ...
    <div className="flex items-start justify-between gap-4 border-b px-4 py-4 sm:px-5">
      ...
      {onEnabledChange ? (
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            {enabled
              ? t("saas.operations.strategy.included")
              : t("saas.operations.strategy.off")}
          </Label>
          <Switch
            aria-label={t("saas.operations.strategy.ruleToggle", {
              title,
              status: enabled
                ? t("saas.operations.strategy.included")
                : t("saas.operations.strategy.off"),
...
export function RuleStrategyConfiguration({
  ...
  const errors = useMemo(() => {
    ...
    if (values.symbols.length === 0)
      next.symbols = t("saas.operations.strategy.errors.selectMarket");
    if (
      !Number.isFinite(values.fastMa) ||
      values.fastMa < 2 ||
      values.fastMa > 200
    )
      next.fastMa = t("saas.operations.strategy.errors.wholeNumber", {
        min: 2,
        max: 200,
      });
    if (
      !Number.isFinite(values.slowMa) ||
      values.slowMa < 3 ||
      values.slowMa > 400
    )
      next.slowMa = t("saas.operations.strategy.errors.wholeNumber", {
        min: 3,
        max: 400,
      });
    if (values.fastMa >= values.slowMa)
      next.slowMa = t("saas.operations.strategy.errors.slowAverage");
    if (values.rsiEnabled) {
      if (values.rsiPeriod < 2 || values.rsiPeriod > 100)
        next.rsiPeriod = t("saas.operations.strategy.errors.wholeNumber", {
          min: 2,
          max: 100,
        });
      if (values.rsiOversold < 1 || values.rsiOversold >= values.rsiOverbought)
        next.rsiOversold = t(
          "saas.operations.strategy.errors.belowUpperThreshold",
        );
      if (
        values.rsiOverbought > 99 ||
        values.rsiOverbought <= values.rsiOversold
      )
        next.rsiOverbought = t(
          "saas.operations.strategy.errors.aboveLowerThreshold",
        );
    }
    if (values.bollingerEnabled) {
      if (values.bollingerPeriod < 2 || values.bollingerPeriod > 100)
        next.bollingerPeriod = t(
          "saas.operations.strategy.errors.wholeNumber",
          { min: 2, max: 100 },
        );
      if (values.bollingerDeviation < 0.5 || values.bollingerDeviation > 5)
        next.bollingerDeviation = t(
          "saas.operations.strategy.errors.valueRange",
          { min: 0.5, max: 5 },
        );
    }
    if (values.momentumEnabled) {
      if (values.macdFast < 2 || values.macdFast > 100)
        next.macdFast = t("saas.operations.strategy.errors.wholeNumber", {
          min: 2,
          max: 100,
        });
      if (
        values.macdSlow < 3 ||
        values.macdSlow > 200 ||
        values.macdSlow <= values.macdFast
      )
        next.macdSlow = t(
          "saas.operations.strategy.errors.largerThanFastPeriod",
        );
      if (values.macdSignal < 2 || values.macdSignal > 100)
        next.macdSignal = t("saas.operations.strategy.errors.wholeNumber", {
          min: 2,
          max: 100,
        });
    }
    if (
      values.takeProfitEnabled &&
      (values.takeProfit <= 0 || values.takeProfit > 100)
    )
      next.takeProfit = t("saas.operations.strategy.errors.percentRange", {
        min: 0.1,
        max: 100,
      });
    if (
      values.stopLossEnabled &&
      (values.stopLoss <= 0 || values.stopLoss > 100)
    )
      next.stopLoss = t("saas.operations.strategy.errors.percentRange", {
        min: 0.1,
        max: 100,
      });
    if (
      ...
      next.maximumPositions = t("saas.operations.strategy.errors.wholeNumber", {
        min: 1,
        max: 100,
      });
    if (
      ...
      values.initialCapital > 100_000_000
    )
      next.initialCapital = "请输入 1 至 100,000,000 USDT 之间的初始模拟资金。";
    if (
      !Number.isFinite(values.orderQuoteAmount) ||
      values.orderQuoteAmount <= 0 ||
      values.orderQuoteAmount > 100_000_000
    )
      next.orderQuoteAmount =
  ...
  const update = <Key extends keyof StrategyFormValues>(
    key: Key,
  ...
  >(
    section: Section,
    key: Key,
    value: AdvancedRuleSetConfig[Section][Key],
  ) => {
    setValues((current) => ({
      ...current,
      advancedRules: {
  ...
  const applyPaperDemoPreset = () => {
    ...
    setName("纸面交易演示策略");
    toast.success(
      "极限演示预设已回填。请保存策略后重新启动；行情数据就绪后会买入，下一轮扫描会模拟卖出。",
    );
  };
  ...
  const activeFilters = [
    ...
    values.momentumEnabled
      ? `MACD ${values.macdFast}/${values.macdSlow}/${values.macdSignal}`
      : null,
  ].filter(Boolean);
  const isPending =
  ...
  return (
    <div
      className={cn(
        "flex flex-col",
        embedded ? "gap-4" : "scroll-container size-full bg-background",
      )}
    >
      <header
        className={cn(
          "border-b px-4 py-4 sm:px-6",
          embedded && "rounded-lg border border-sky-400/15 bg-card/90",
        )}
      >
        <div
          className={cn(
            "mx-auto flex max-w-[1600px] flex-col gap-4 lg:flex-row lg:items-center lg:justify-between",
            embedded && "max-w-none",
          )}
        >
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <ConfigurationHeading className="font-semibold text-xl">
                {t("saas.operations.strategy.title")}
      ...
      >
        ...
        <aside className="flex min-w-0 flex-col gap-4 xl:sticky xl:top-0 xl:self-start">
          <Card className="gap-0 rounded-lg py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4">
              <CardTitle className="flex items-center gap-2 text-base">
                <Layers3 /> {t("saas.operations.strategy.summary.title")}
              </CardTitle>
              <CardDescription>
                {t("saas.operations.strategy.summary.description")}
              </CardDescription>
            </CardHeader>
            <CardContent className="divide-y px-4 py-2" aria-live="polite">
              <SummaryRow
                label={t("saas.operations.strategy.summary.paperMarkets")}
                value={
                  values.symbols.join(", ") ||
                  t("saas.operations.strategy.summary.noneSelected")
                }
              />
              <SummaryRow
                label={t("saas.operations.strategy.summary.timeframe")}
                value={values.timeframe}
              />
              <SummaryRow
                label={t("saas.operations.strategy.summary.entry")}
                value={t("saas.operations.strategy.summary.entryValue", {
                  fast: values.fastMa,
                  slow: values.slowMa,
                })}
              />
              <SummaryRow
                label={t("saas.operations.strategy.summary.confirmations")}
                value={
                  activeFilters.join(" | ") ||
                  t("saas.operations.strategy.summary.none")
                }
              />
              <SummaryRow
                label="初始模拟资金"
                value={`${values.initialCapital.toLocaleString()} USDT`}
              />
              <SummaryRow
                label="单笔开仓金额"
                value={`${values.orderQuoteAmount.toLocaleString()} USDT；余额不足则阻塞`}
              />
              <SummaryRow
                label={t("saas.operations.strategy.summary.exitLimits")}
                value={t("saas.operations.strategy.summary.exitLimitsValue", {
                  takeProfit: values.takeProfit,
                  stopLoss: values.stopLoss,
                })}
              />
              <SummaryRow
                label={t("saas.operations.strategy.summary.exposureCap")}
                value={t("saas.operations.strategy.summary.exposureCapValue", {
                  positions: values.maximumPositions,
                  leverage: values.leverage,
                })}
              />
            </CardContent>
          ...
          <Card className="gap-0 rounded-lg py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4">
              <CardTitle className="flex items-center gap-2 text-base">
                <BrainCircuit /> AI 策略审阅
              </CardTitle>
              <CardDescription>
                在独立页面阅读完整中文分析与确定性评估证据。
              </CardDescription>
            </CardHeader>
            <CardContent className="px-4 py-4">
              {strategyId ? (
                <Button
                  ...
                  variant="outline"
                >
                  <Link to="/strategies/advisory">
                    <BrainCircuit /> 打开 AI 策略审阅
                  </Link>
                </Button>
              ) : (
                <Button
          ...
          {storedStrategy ? (
            <Card className="gap-0 rounded-lg py-0 shadow-none">
              <CardHeader className="border-b px-4 py-4">
                ...
                <CardDescription>
                  Server-recorded paper balance and marked equity.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-2 gap-x-4 gap-y-3 px-4 py-4 text-sm">
                ...
                <SummaryRow
                  label="Realized PnL"
                  value={`${storedStrategy.account.realized_pnl_quote.toFixed(2)} USDT`}
                />
                <SummaryRow
                  label="Unrealized PnL"
                  value={`${storedStrategy.account.unrealized_pnl_quote.toFixed(2)} USDT`}
                />
              </CardContent>
            </Card>
          ) : null}
          ...
          <Card className="gap-0 rounded-lg border-sky-500/30 py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4">
              <CardTitle className="flex items-center gap-2 text-base">
                <WandSparkles className="text-sky-500" /> AI 策略文本导入
              </CardTitle>
              <CardDescription>
                粘贴自然语言策略。AI
                只会生成待审核的参数草稿，不会保存、启动或下单。
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 px-4 py-4">
              <Textarea
