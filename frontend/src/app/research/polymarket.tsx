  ...
  return (
    <div className="grid gap-3 border-y bg-muted/30 px-4 py-3 text-xs sm:grid-cols-[auto_1fr_auto_auto] sm:items-center">
      ...
      <span className="text-muted-foreground">
        Source timestamp: {formatTimestamp(metadata?.source_timestamp_ms)} |
        Observed: {formatTimestamp(metadata?.observed_at_ms)}
      </span>
      <span className="text-muted-foreground">
  ...
  return (
    <Table>
      <TableHeader>
        <TableRow>
          <TableHead>
            {side === "bid" ? "Bid probability" : "Ask probability"}
          </TableHead>
...
export default function PolymarketResearch() {
  ...
  const unavailable =
    Boolean(catalog.error || snapshot.error) ||
    (!catalog.isLoading && activeMarkets.length === 0);
          ...
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px]">
            <Select
              ...
              onValueChange={(nextMarketId) => {
                const nextMarket = activeMarkets.find(
                  (market) => market.market_id === nextMarketId,
                );
            ...
            >
              <SelectTrigger aria-label="Market selector">
                <SelectValue placeholder="Select a public market" />
              </SelectTrigger>
              <SelectContent>
            ...
            >
              <SelectTrigger aria-label="Outcome selector">
                <SelectValue placeholder="Select outcome" />
              </SelectTrigger>
              <SelectContent>
                {selectedMarket?.outcomes.map((item) => (
                  <SelectItem key={item.token_id} value={item.outcome}>
                    {item.outcome}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Alert>
              ...
              <AlertTitle>
                {activeMarkets.length === 0
                  ? "No active public markets are available"
                  : "Public market data is unavailable"}
              </AlertTitle>
              <AlertDescription>
                The workstation retained no inferred prices. Check source
                freshness and retry; research and replay controls stay disabled
                until a public snapshot is returned.
              </AlertDescription>
            </Alert>
          ) : null}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              [
                ...
                formatProbability(book?.microprice ?? book?.midpoint),
                "Microprice when available",
              ],
              [
                "Best bid",
                formatProbability(book?.best_bid),
                "Public CLOB bid",
              ],
              [
                "Best ask",
                formatProbability(book?.best_ask),
                "Public CLOB ask",
              ],
              [
                "Book condition",
                book?.health?.status ?? (loading ? "Loading" : "Unknown"),
                book?.health?.reason ?? "Validate before interpreting",
              ],
          ...
          <Card className="gap-0 rounded-lg py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4 sm:px-5">
              <CardTitle className="flex items-center gap-2 text-base">
                <Layers3 /> Public order-book depth
              </CardTitle>
              <CardDescription>
                Displayed levels are observations, not executable quotes or
        ...
        <aside className="flex flex-col gap-4">
          <Card className="gap-0 rounded-lg py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4">
              <CardTitle className="flex items-center gap-2 text-base">
                <Activity /> Research signal
              </CardTitle>
              <CardDescription>
                Probability movement summary from locally observed public
                snapshots.
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="mt-1 font-medium tabular-nums">
                    {formatProbability(detail?.signal?.reference_price)}
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">Volatility</p>
                  <p className="mt-1 font-medium tabular-nums">
                    {formatNumber(detail?.signal?.volatility, 6)}
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">Method</p>
                  <p className="mt-1 font-medium">
                    {detail?.signal?.reference_method ??
                ...
                <div>
                  ...
                  <p className="mt-1 font-medium tabular-nums">
                    {detail?.signal?.observation_count ?? history.length}
                  </p>
                </div>
              </div>
              <Alert>
                <Gauge />
                <AlertTitle>
                  {detail?.signal?.volatility_status ?? "Caveat"}
                </AlertTitle>
                <AlertDescription>
                  Signals describe observed probability variation only. They do
                  not estimate outcome likelihood or performance.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card className="gap-0 rounded-lg py-0 shadow-none">
            <CardHeader className="border-b px-4 py-4">
              ...
              <CardDescription>
                Deterministic simulation against the received public book; no
            ...
            <CardContent className="px-4 py-4">
              <form className="grid gap-3" onSubmit={runReplay}>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <SelectTrigger aria-label="Paper replay side">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="buy">Paper buy</SelectItem>
                      <SelectItem value="sell">Paper sell</SelectItem>
                    </SelectContent>
                  </Select>
                  <Input
                    aria-label="Paper replay size"
                    inputMode="decimal"
                    min="0"
                ...
                <Input
                  aria-label="Assumed latency milliseconds"
                  inputMode="numeric"
                  min="0"
                ...
                >
                  <TestTube2 />{" "}
                  {replay.isPending ? "Simulating" : "Run paper replay"}
                </Button>
              </form>
              {replay.error ? (
                ...
                <div className="mt-4 grid gap-3 border-t pt-4 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium tabular-nums">
                      {replay.data.fill.vwap === null
                        ? "--"
                        : `${numberFixed(replay.data.fill.vwap * 100, 2)}%`}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">
                    ...
                    <span className="font-medium tabular-nums">
                      {numberFixed(replay.data.fill.filled_size, 2)} /{" "}
                      {numberFixed(replay.data.fill.requested_size, 2)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">
                    ...
                    <span className="font-medium tabular-nums">
                      {numberFixed(replay.data.fill.unfilled_size, 2)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Marked P&L</span>
                    <span className="font-medium tabular-nums">
                      {numberFixed(replay.data.mark_to_book.pnl, 4)}{" "}
                      {replay.data.mark_to_book.currency}
                    </span>
                  </div>
                  <Badge className="w-fit" variant="outline">
                    <TestTube2 /> {replay.data.simulation_mode} |{" "}
                    {Number(replay.data.fill.unfilled_size) > 0
                      ? "Partial fill"
                      : "Full fill"}
                  </Badge>
                </div>
          ...
          <Alert>
            ...
            <AlertDescription>
              Public source data may be delayed, incomplete, stale, or
              unavailable. This screen cannot access a wallet and cannot place a
              live order.
            </AlertDescription>
          </Alert>
        </aside>
