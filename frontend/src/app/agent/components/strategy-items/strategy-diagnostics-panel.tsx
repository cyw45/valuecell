...
const StrategyDiagnosticsPanel: FC<StrategyDiagnosticsPanelProps> = ({
  ...
  const latestCurvePoint =
    priceCurve.length > 1 ? priceCurve[priceCurve.length - 1] : undefined;
  const latestCurveValue = latestCurvePoint?.[1];
  const latestEquity =
    typeof latestCurveValue === "number"
      ? latestCurveValue
      : summary?.total_value;
  ...
  const marketIsHealthy =
    health?.ok ?? (expectedCount > 0 && observedCount >= expectedCount);
  const pnlTone =
    pnlType === "positive"
      ? "positive"
      : pnlType === "negative"
    ...
    <div className="flex flex-col gap-5">
      ...
      {showOverview && (
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 2xl:grid-cols-4">
          <RuntimeMetricCard
            label={t("strategy.diagnostics.runtime", {
              defaultValue: "运行状态",
            })}
            value={statusLabel}
            caption={
              diagnostics?.exchange_id ?? strategy?.exchange_id ?? EMPTY_VALUE
            }
            icon={
              isRunning ? (
          ...
          <RuntimeMetricCard
            ...
            caption={
              latestCurveTime
                ...
                : t("strategy.diagnostics.waitingCurve", {
                    defaultValue: "等待资产曲线",
                  })
            }
          ...
          <RuntimeMetricCard
            ...
            caption={
              health?.missing_count
                ? `${t("strategy.diagnostics.missingSymbols", { defaultValue: "缺失行情" })}: ${health.missing_symbols.join(", ")}`
                : t("strategy.diagnostics.marketReady", {
                    defaultValue: "行情覆盖正常",
                  })
            }
            icon={
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              {showExecution
                ? t("strategy.diagnostics.latestDecision", {
                    defaultValue: "最近决策",
                  })
            ...
            <div className="overflow-x-auto rounded-lg border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>
                      {t("strategy.diagnostics.symbol", {
                        defaultValue: "币种",
                      })}
                    </TableHead>
                    <TableHead>
                      {t("strategy.diagnostics.snapshot", {
                        defaultValue: "实时行情",
                      })}
                    </TableHead>
                    <TableHead>
                      {t("strategy.diagnostics.price", {
                        defaultValue: "价格",
                      })}
                    </TableHead>
                    <TableHead>
                      {t("strategy.diagnostics.action", {
                        defaultValue: "动作",
                      })}
                    </TableHead>
                ...
                <TableBody>
                  {symbolRows.length > 0 ? (
                    symbolRows.map((item) => {
                      const positionPnlType = getChangeType(
                        item.position?.unrealized_pnl,
                      );
                      return (
                        <TableRow key={item.symbol}>
                          ...
                          <TableCell>
                            <span
                              style={{ color: stockColors[positionPnlType] }}
                            >
                              {item.position
                                ? `${formatChange(item.position.unrealized_pnl, "", 2)} (${formatChange(item.position.unrealized_pnl_pct, "", 2)}%)`
                                : EMPTY_VALUE}
                            </span>
                          </TableCell>
      ...
      {showExecution && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              {t("strategy.diagnostics.executionExplanation", {
                defaultValue: "本轮执行解释",
              })}
            </CardTitle>
            <CardDescription>
          ...
          <CardContent className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(16rem,0.8fr)]">
            <div className="space-y-2">
              <p className="font-medium text-sm">
                {t("strategy.diagnostics.triggered", {
                  defaultValue: "触发条件",
                })}
              </p>
                ...
                triggeredConditions.map((condition, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-emerald-600 text-xs dark:text-emerald-300">
                        {condition.status}
                      </span>
                    </div>
            ...
            <div className="space-y-2">
              <p className="font-medium text-sm">
                {t("strategy.diagnostics.notTriggered", {
                  defaultValue: "未触发条件",
                })}
              </p>
              {blockedConditions.length > 0 ? (
                blockedConditions.map((condition, index) => (
                  <div
                    key={`${condition.label}-${index}`}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-amber-600 text-xs dark:text-amber-300">
                        {condition.status}
                      </span>
                    </div>
                    <p className="mt-1 text-muted-foreground text-xs">
            ...
            <div className="space-y-3">
              <div>
                <p className="font-medium text-sm">
                  {t("strategy.diagnostics.actionReason", {
                    defaultValue: "动作理由",
                  })}
                </p>
                <p className="mt-1 text-muted-foreground text-sm">
                  {formatValue(explanation.action_reason)}
                </p>
              </div>
              <div>
                <p className="font-medium text-sm">
                  {t("strategy.diagnostics.fundImpact", {
                    defaultValue: "资金影响",
                  })}
                </p>
      ...
      {showExecution && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              {t("strategy.diagnostics.symbolExplanation", {
                defaultValue: "币种决策明细",
              })}
            </CardTitle>
          </CardHeader>
                            ...
                            <span
                              className={
                                ...
                                condition.status === "passed"
                                  ? "shrink-0 font-medium text-emerald-600 dark:text-emerald-300"
                                  : "shrink-0 font-medium text-amber-600 dark:text-amber-300"
                              }
                            >
                              {condition.status}
                            </span>
      ...
      {showConfiguration && (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                {t("strategy.diagnostics.config", {
                  defaultValue: "实际生效配置",
                })}
              </CardTitle>
              <CardDescription>
                {t("strategy.diagnostics.configDesc", {
                  defaultValue: "后端已保存且用于执行的策略、金额和仓位参数。",
                })}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                {configEntries.length > 0 ? (
                  ...
                  <div className="rounded-md border border-dashed bg-muted/30 p-4 text-muted-foreground text-sm md:col-span-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                {t("strategy.diagnostics.latestCycle", {
                  defaultValue: "最近扫描",
                })}
              </CardTitle>
            ...
            <CardContent className="space-y-3">
              <div className="rounded-md border bg-muted/30 p-3 text-sm leading-relaxed">
                {latest?.rationale ||
                  t("strategy.diagnostics.noRationale", {
                    defaultValue: "暂无本轮理由。",
                  })}
              </div>
              <div className="grid grid-cols-3 gap-2 text-center text-sm">
                ...
                <div className="rounded-md bg-muted/40 p-3">
                  ...
                  <p className="mt-1 font-semibold">
                    {latest?.no_order_count ?? 0}
                  </p>
                </div>
                <div className="rounded-md bg-muted/40 p-3">
                  <p className="text-muted-foreground text-xs">cycles</p>
                  <p className="mt-1 font-semibold">
                    {diagnostics?.recent_cycles.length ?? 0}
                  </p>
                </div>
              </div>
            </CardContent>
