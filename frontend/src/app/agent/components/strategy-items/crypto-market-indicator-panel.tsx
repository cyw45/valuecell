...
import { useGetCryptoMarketIndicators, useGetCryptoSymbols } from "@/api/crypto-market";
...
const buildChartOption = (data?: CryptoSymbolIndicators): EChartsOption => {
  ...
  const seriesFor = (selector: (point: NonNullable<CryptoSymbolIndicators["indicators"]>[number]) => number | null | undefined) =>
    candles.map((item) => {
  ...
  return {
    ...
    xAxis: [
      { type: "category", data: categories, boundaryGap: true, axisLabel: { show: false } },
      { type: "category", data: categories, gridIndex: 1, axisLabel: { show: false } },
      { type: "category", data: categories, gridIndex: 2, axisLabel: { show: false } },
    ...
    series: [
      ...
      { name: "Close", type: "line", data: closeValues, showSymbol: false, smooth: true },
      { name: "MA5", type: "line", data: seriesFor((item) => item.ma.ma5), showSymbol: false },
      { name: "MA20", type: "line", data: seriesFor((item) => item.ma.ma20), showSymbol: false },
      { name: "MA60", type: "line", data: seriesFor((item) => item.ma.ma60), showSymbol: false },
      { name: "BB Upper", type: "line", data: seriesFor((item) => item.bollinger.upper), showSymbol: false, lineStyle: { type: "dashed" } },
      { name: "BB Mid", type: "line", data: seriesFor((item) => item.bollinger.middle), showSymbol: false, lineStyle: { type: "dashed" } },
      { name: "BB Lower", type: "line", data: seriesFor((item) => item.bollinger.lower), showSymbol: false, lineStyle: { type: "dashed" } },
      { name: "RSI", type: "line", xAxisIndex: 1, yAxisIndex: 1, data: seriesFor((item) => item.rsi), showSymbol: false, smooth: true },
      { name: "Momentum", type: "line", xAxisIndex: 2, yAxisIndex: 2, data: seriesFor((item) => item.momentum), showSymbol: false, smooth: true },
      { name: "MACD", type: "line", xAxisIndex: 2, yAxisIndex: 2, data: seriesFor((item) => item.macd), showSymbol: false },
      { name: "MACD Signal", type: "line", xAxisIndex: 2, yAxisIndex: 2, data: seriesFor((item) => item.macd_signal), showSymbol: false },
      { name: "Volume", type: "line", xAxisIndex: 3, yAxisIndex: 3, data: volumes, showSymbol: false, lineStyle: { opacity: 0.45 } },
    ],
  };
};
...
const CryptoMarketIndicatorPanel: FC<CryptoMarketIndicatorPanelProps> = ({
  ...
  const preferredSymbols = normalizedStrategySymbols.length > 0
        ...
        <CardHeader className="gap-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <CardDescription>
                加密货币 USDT 交易对，实时拉取 K线并计算 MA、RSI、布林带、动能/MACD。
              </CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant={isFetching ? "outline" : "secondary"} className="gap-1">
          ...
          <div className="flex flex-wrap gap-2">
            {(preferredSymbols.length > 0 ? preferredSymbols : allSymbols).slice(0, 18).map((item) => (
              <Button
                key={item}
                type="button"
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            <div className="rounded-lg border bg-muted/30 p-3">
              ...
              <p className="mt-1 font-semibold text-lg">{numberFixed(current?.latest_price ?? undefined, 4)}</p>
            </div>
            <div className="rounded-lg border bg-muted/30 p-3">
              <p className="text-muted-foreground text-xs">RSI</p>
              <p className="mt-1 font-semibold text-lg">{numberFixed(latestIndicator?.rsi ?? undefined, 2)}</p>
            </div>
            <div className="rounded-lg border bg-muted/30 p-3">
              <p className="text-muted-foreground text-xs">Momentum</p>
              <p className="mt-1 font-semibold text-lg">{numberFixed(latestIndicator?.momentum ?? undefined, 4)}</p>
            </div>
            <div className="rounded-lg border bg-muted/30 p-3">
              <p className="text-muted-foreground text-xs">MACD Hist</p>
              <p className="mt-1 font-semibold text-lg">{numberFixed(latestIndicator?.macd_histogram ?? undefined, 4)}</p>
            </div>
          </div>
          <div className={cn("rounded-lg border bg-background", !current && "flex h-[520px] items-center justify-center text-muted-foreground")}> 
            {current ? <IndicatorChart data={current} /> : error ? "行情暂不可用，请稍后重试。" : "等待行情数据..."}
          </div>
        </CardContent>
      </Card>
    </div>
