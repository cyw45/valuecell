...
6. 每个策略的交易统计：成交笔数、完整交易次数、胜率、累计成交额、手续费和资金周转率；没有完整卖出时胜率显示 `—`，不能显示伪造的 0%。
7. OKX 共享钱包权益曲线只使用后台持久化的钱包快照；曲线的最新金额应与对应时间的钱包事实一致，不能用任何单策略 PnL 代替。
8. 共享账户门禁明确显示“可开仓”“只读保护”或“开仓已阻断”，并展示服务端返回的每一条原因；不能只看策略运行状态判断是否会下单。
9. 没有数据时显示 `—`、`不可用` 或明确原因，不得显示伪造的 `0`。
10. Paper 账户和 Paper PnL 不出现在 Demo 共享钱包汇总中。
